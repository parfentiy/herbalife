<head>
    <title>Список заказов за {{$period}}</title>
...
<div class="container d-flex flex-row justify-content-center">
    <form name="period" method="post" action="{{ url('/neworder') }}">
        @csrf
        <td style="border: 1px solid; text-align: center;">
            <label for="period">Месяц</label>
            <input type="month" name="period" value="{{date("Y-m", strtotime($period))}}" required>
            <button class="btn btn-primary btn-sm" type="submit" name="action" value="changed">Изменить</button>
        </td>
    </form>
</div>
...
<x-paginate-orders :period="$period"/>
</body>
